        ...
        loadedEntries.forEach { entry ->
            ...
            if (effectiveUuid != null) {
                UsernameResolver.resolveUuid(effectiveUuid.toString()).thenAccept { resolvedName ->
                    ...
                    rebuildSnapshot()
                    PlayerListMod.logger.info("Synced custom player username: {} -> {}", effectiveUuid, normalizedName)
                }
                return@forEach
            }
            ...
            UsernameResolver.resolve(requestedUsername).thenAccept { resolved ->
                ...
                rebuildSnapshot()
                PlayerListMod.logger.info("Synced custom player UUID: {} -> {}", requestedUsername, resolvedUuid)
            }
        }
    }
    ...
    private fun toCustomization(entry: ContentManager.LoadedPlayerCustomization): PlayerCustomization? {
        val uuid = entry.uuid?.let { value ->
            runCatching { UUID.fromString(value) }
                .getOrElse {
                    PlayerListMod.logger.warn("Skipping customization '{}' because uuid '{}' is invalid", entry.username, value)
                    return null
                }
        }
        val normalizedUsername = entry.username.trim()
        if (normalizedUsername.isEmpty() && uuid == null) {
            PlayerListMod.logger.warn("Skipping customization because both username and uuid are missing")
            return null
        }
